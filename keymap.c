...
tap_dance_action_t tap_dance_actions[] = {
  ...
  [TD_HOME]  = ACTION_TAP_DANCE_DOUBLE(KC_HOME, C(KC_TAB))
};
...
#define END LT(3,KC_NO)        // End and Numlock
...
[_WORKMAN] = LAYOUT(
  ...
  UNDO,          KC_Z,         KC_X,         KC_M,         KC_C,         KC_V,   KC_QWERTY,              MUTESLEEP, KC_K,    KC_L,         KC_COMM,      KC_DOT,       KC_SLSH,       REDO,
                               HOME,         END,       KC_ESCNAV,    KC_SPC, LCTL_T(KC_TAB),            KC_ENTSYM,  KC_BSNM, KC_DELFUN,    KC_PGUP,     LGUI_T(KC_PGDN)
),
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        ...
        case LT(5,KC_NO):      // Mute and Sleep
            ...
            return false;
            break;
        case LT(3,KC_NO):      // End and Numlock
            if (record->tap.count && record->event.pressed) {
...
const uint16_t PROGMEM encoder_map[][NUM_ENCODERS][NUM_DIRECTIONS] = {
    ...
    [_SYMBL] =  {    ENCODER_CCW_CW(KC_END, KC_HOME),           ENCODER_CCW_CW(XXXXXXX, XXXXXXX)  },
